...
if [ -f .env ]; then
    ...
    read -r -p "是否覆盖并重新运行配置向导？ [y/N]: " choice
    case "$choice" in
        [yY][eE][sS]|[yY])
            echo -e "${BLUE}开始重新配置...${NC}\n"
            ;;
        *)
            RECONFIGURE=false
            echo -e "${GREEN}保持现有 .env 配置不变。${NC}\n"
            ;;
    esac
fi
...
case "$deploy_choice" in
    ...
    *)
        ...
        if grep -q "^REDIS_ADDR=" .env; then
            echo -e "  ${BLUE}$COMPOSE_CMD --profile with-redis up -d${NC}"
        else
